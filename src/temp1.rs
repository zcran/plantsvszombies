...
fn main() {
    App::new()
        ...
        .register_ldtk_entity::<PlayerBundle>("Player") // 初始化玩家实体
        .register_ldtk_entity::<GoalBundle>("Goal") // 初始化目标实体
        .add_systems(
            Update,
            (
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    camera.transform.translation.x += 1280.0 / 4.0; // 相机位置
...
fn move_player_from_input(
    ...
    for mut player_grid_coords in players.iter_mut() {
        ...
        if !level_walls.in_wall(&destination) { // 判断在地图内移动
...
fn translate_grid_coords_entities(  // 转化网格坐标实体的位置
